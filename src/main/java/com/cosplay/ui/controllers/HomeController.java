    ...
    private void loadHeroBanner() {
        ...
        bannerDAO.getActiveBanner().ifPresentOrElse(
            banner -> {
                ...
                // Apply title color
                String titleColor = banner.getTitleColor() != null ? banner.getTitleColor() : "#FFFFFF";
                heroBannerTitle.setStyle("-fx-font-size: 24px; -fx-font-weight: bold; -fx-text-fill: " + titleColor + "; -fx-padding: 5 20; -fx-effect: dropshadow(gaussian, rgba(0, 0, 0, 0.6), 4, 0, 0, 1);");
                ...
                String subtitleColor = banner.getSubtitleColor() != null ? banner.getSubtitleColor() : "#FFFFFF";
                heroBannerMessage.setStyle("-fx-font-size: 15px; -fx-text-fill: " + subtitleColor + "; -fx-padding: 0 20; -fx-effect: dropshadow(gaussian, rgba(0, 0, 0, 0.6), 4, 0, 0, 1);");
                ...
                if (banner.getImagePath() != null && !banner.getImagePath().isBlank()) {
                    try {
                        ...
                        if (bgImage != null && !bgImage.isError()) {
                            ...
                            // Center the image using viewport for better aspect ratio handling
                            double imgWidth = bgImage.getWidth();
                            double imgHeight = bgImage.getHeight();
                            double containerWidth = 820.0;
                            double containerHeight = 190.0;
                            
                            // Calculate scale to cover the container while maintaining aspect ratio
                            ...
                            double viewportX = (imgWidth - viewportWidth) / 2;
                            double viewportY = (imgHeight - viewportHeight) / 2;
                            
                            // Set viewport to crop and center the image
                            bannerImageView.setViewport(new javafx.geometry.Rectangle2D(
